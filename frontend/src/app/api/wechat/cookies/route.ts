...
export async function GET(request: Request) {
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/wechat/cookies?${new URLSearchParams(params)}`);
...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/wechat/cookies`, {
      method: "POST",
